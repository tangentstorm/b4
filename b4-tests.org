...
* DONE [3/5] memory operations

** TODO: test that %C clears memory and stacks

** DONE =wv= op

By default we write a 32-bit integer:
...
Invoking =vb= changes the value width to one byte:
...
** DONE =rv= op : read a value from address

By default, we read a single byte:
...
But the =vi= op sets the value width to 4 bytes.
...
The =vb= op sets the value width back to 1 byte.
...
The + ops increment a register by the value width, so we can use them as iterators through an array of values.

#+name: op.plus-x
#+begin_src b4a
> 11223344 !X
> +X +X @X ?d %q
ds: [11223344 11223348 1122334C]
#+end_src
...
> vb 11223344 !X
> +X +X @X ?d %q
ds: [11223344 11223345 11223346]
#+end_src

Note that @X !X and +X transfer a complete register (32 bits) regardless of the value size.
=vb= and =vi= affect the =rv=, =wv=, and =+x= ops, but for =+x= they only change by how much the value increments.
...
#+begin_src b4a
  ...
  00000100
  > ?_                # and ^_ should reflect the 4 bytes we assembled
  00000104
#+end_src
