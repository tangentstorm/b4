...
- =/R= : Reset VM (clear stacks, set IP to 0x100)
- =/C= : Clear VM completely (boot + clear dictionary)
...
** Dictionary/Debug
- =/p= : Print dictionary (all defined words)
...
The B4-native implementation ([[file:../b4a/b4i.b4a][b4a/b4i.b4a]]) can run on any VM by spawning a Level 0 implementation as a child process.
