...
* use utf-8 as organizing scheme for opcodes
...
- originally i assigned =c2= to C2, =n1= to C3, =c4= to C4, but eventually
  remembered these hex codes are used by utf-8.
...
* command sigils
Originally =%= was the prefix for some commands, and then later others used =\= , and there was no clear rule to distinguish them.

I wanted to unify on one, and prefer =\= , but at some point, I would like the on-board editor to use =\= as a prefix for typing non-ascii characters.

I am also strongly considering =%= as a prefix for decimal integers, since most numbers in b4 are hex.

So instead, I settled on =/= for commands.
...
- no, this is wrong. you can always fetch the new value with =@R= later
