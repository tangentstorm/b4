...
** DONE [4/4] basic control flow templates, based on the basic programs from [[#APTOP]]
*** note: register =Z= contains the address we're writing to as we assemble.
*** DONE =ok= is ~[ ].~ 'ok' does nothing, so no bytecode is emitted.
*** DONE =if .. th  .. el .. fi= provides conditionals
**** =if= is ~[ ( - ) ]~
"If" is pretty much a no-op and only there for human readers.
**** =th= ("then")  is ~[ ( -z) si h0 zw si 00 zw zd ]~
...
**** =el= ("else") is ~[ (a-z) si hp zw     si 00 zw     zd    fi ]~
...
**** =^b= (helper macro) writes a single byte to an address, while preserving Z
 ...
 zw (.. z) and M[a]=b
 dz (.. ) restoring Z
*** DONE =wh .. do .. od= provides while loops
**** =wh= is ~[ (  -w) zd ]~
**** =do= is ~[ (  -d) th ]~
**** =od= is ~[ (wd- ) sw si hp zw zd sw su zw fi ]~
(wd-dw)  =sw=
