...
Using =:address= sets the assembly position to a specific address.
Using just =:= (with no address) continues assembling from where
the previous line left off. This allows multi-line assembly without
having to manually track addresses.
...
=;= always exits the assembler (back to calculator mode) and
invokes =^]= (which by default simply returns). This allows
defining a word and calling it on the same line.
...
=:!name= saves the old HERE pointer, sets HERE to the address
stored in label =name=, and enters the assembler. When =;= is
reached, it exits the assembler and invokes =^]=, which during
a =:!= assembly restores the old HERE pointer.
...
Multiple =:!name ... ;= and =: ... ;= segments can appear on a single
line. Each =;= restores HERE (if =:!= was used) or simply exits the
assembler (if bare =:= was used), and the line continues in immediate
mode.
