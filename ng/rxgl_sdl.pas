...
constructor TSDLVDP.Create;
  begin
    ...
    self.pBitmap := SDL_SETVIDEOMODE(canvas_w, canvas_h, bitdepth,
                                     SDL_HWSURFACE);
...
procedure TSDLVDP.CreateCanvas;
  begin
    self.canvas := TRxCanvas.Create(canvas_w, canvas_h,
                                    pBitmap^.pixels );
  end;
{$ENDIF}
...
procedure TSDLVDP.PlotPixel(adr: Int32; Value: byte); inline;
  ...
  begin
    if not assigned(self.pBitmap) then
      begin
        writeln('pBitmap is nil!');
        halt
      end;
    rBitmap := pBitmap^.pixels + 4 * (self.rVStart * canvas_w +
                                      self.rHStart + adr);
    with rBitmap^ do
      begin
        r := Value;
