...
:word?                             # (s-e)   attempt to find word in dictionary
  ...
  .w du lb 0 eq nt .d              # while e!=0 do:
    ...
    cd du dc                       #  ..- ets|ts
    s-eq? .t cd zp rt .z           # return e if strings match
...
#+begin_src b4a
  ...
  :repl .w lb 01 .d
    li `eol rb .t                    # if end of line flag
      lb 00 li `eol wb b4> .z        # show prompt if new line
    tr                               # read char from terminal
    du lb '` eq .t db .z             # ` = debugger
    du lb 32 eq .t tw                # on space,  (echo space and add dummy char)
      end-token
      du word?                       #    look up the word
      du lb 0 eq .t zp try-num       #    if not found, complain
      .e sw zp exec .z               #    else execute
    .e
      lb 31 ov lt .t du du s+ tw     # copy non-ctrl chars to buffer and echo
      .e du lb 04 eq .t bye .z       # on ^d, exit
      .z zp
    .z .o
#+end_src
...
:s-eq?                             # (st-?) are strings s and t the same?
  ov ov eq .t eq rt .z             # a. if the addresses are the same, return 1
  ov rb du dc ov rb eq nt .t       # b. (st-st?|n) if the lengths are different,
  ...
  cd .f inc sw inc                 # st-TS  -> next char (doesn't matter that we swap order)
     ov rb ov rb eq nt .t          # if the characters are not equal
...
:mm-claim                             # (-a) claim mp (and possibly split end off)
  ...
  mm-split ov lt .t                # (r-r) if worth splitting
    # shrink old block:
...
:mm-alloc                             # (n-a) allocate block of n bytes
  ...
  :mm-loop
    ...
    .e @mp mm-next ri                 # consider p^.next
       du @mp lb 0 eq .t rt           # if next=null return null
