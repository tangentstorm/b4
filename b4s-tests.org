...
* TEST b4s.bios : ensure BIOS is loaded
#+begin_src b4a
...
* TEST b4s.nil-parts : car and cdr (nil)
#+begin_src b4a
  > inic
  > nil car nil?
  > nil cdr nil?
...
* TEST b4s.nxt-ptr : =inic= and =nxt^= : pointer to next free cell
#+begin_src b4a
  > inic
...
* TEST b4s.nxtc : =nxtc= : take next cell from free list
#+begin_src b4a
  > inic
...
* TEST b4s.cons : =cons= : construct a cell
#+begin_src b4a
  > inic
  > ABCD 1234 cons !C
...
* TEST b4s.nlst : =nlst= : combine n values into a list
#+begin_src b4a
 > inic
...
* TEST b4s.nums : =cnum= / =pnum= : construct and print a lisp number
#+begin_src b4a
  > inic
  > 7C cnum du pnum \0
  7C
...
* TEST b4s.sym-helpers : =glu= and =o4c= : symbol helpers
#+begin_src b4a
...
* TEST b4s.syms : =csym= / =psym= : construct and print symbols
#+begin_src b4a
  > inic
  > 't 'a 'c 'o ?d
  ds: [74 61 63 6F]
...
* TEST b4s.ctyp : =ctyp= : distinguish cell types
#+begin_src b4a
  > inic
...
* TEST b4s.pval-num-sym : =pval= 1 : atoms
#+begin_src b4a
  > inic
...
* TEST b4s.pval-pair : =pval= 3 : dotted pairs
#+begin_src b4a
  > inic
...
* TEST b4s.pval-list : =pval= 4 : lists of atoms
#+begin_src b4a
  > inic
...
* TEST b4s.pval-nested : =pval= 5 : nested lists
#+begin_src b4a
  > inic
...
* TEST b4s.pval-quotes : =wquo=, =wqup=, =wqdn=: quotation builders
#+begin_src b4a
  > inic
  > CA cnum wquo pval \0
  > CB cnum wqup pval \0
...
* TEST b4s.rlis : =node=, =newc=, =acq!=, and =done= : imperative list builder
#+begin_src b4a
  > inic
...
* TEST b4s.rval-atoms : =rval= 1: atoms
#+begin_src b4a
  > inic
  > :S ..
  > C0 pre rval ?d /R
...
* TEST b4s.rval-quoted : =rval= 2: quoted atoms
#+begin_src b4a
  > inic
  > :S '' '0 ..
...
* TEST b4s.rval-nil : =rval= 3: the empty list
#+begin_src b4a
  > inic
  > :S '( ') ..
...
* TEST b4s.rval-atom-lists : =rval= 4: lists with one atom
#+begin_src b4a
  > inic
  > :S "(0)" ..
  > C0 pre rval sw pval ?d \0 /R
...
* TEST rval-longer-lists : =rval= 5: longer lists
...
* TEST rval-nested : =rval= 6: nested lists
#+begin_src b4a
  > inic
  > :S "(())" ..
  > C0 pre rval sw pval ?d \0 /R
...
* TEST b4s.rval-nested-nils : =rval= 7: nested nils
#+begin_src b4a
  > inic
  > :S "(())" ..
  > C0 pre rval sw pval ?d \0 /R
...
* TEST b4s.rval-dotted : =rval= 8: dotted pairs
#+begin_src b4a
  > inic
  > :S "(a . 1)" ..
  > C0 pre rval sw pval ?d \0 /R
...
* TEST b4s.rval-quo : =rval= 9: explicit quotes
#+begin_src b4a
  > inic
  > :S "(quo . C0)" ..
...
* TEST nlist-print : =nlst= (build list from n cells)
#+begin_src b4a
 > inic
 > # construct the list: (AB CD efgh)
...
* TEST nlist-cnav : =cnav= (use hex number as path through cells)
#+begin_src b4a
> :S "(A (B . C) (D E (F)))" ..
...
* TEST b4s.eq-p : =eq?=  (=eq= is a b4 opcode)
#+begin_src b4a
> inic
...
* TEST b4s.csym1 : =symc= - symbol from char
#+begin_src b4a
  > :S 'a  .. rt
  > inic
...
* TEST b4s.apnd : =link= : append one list to another
#+begin_src b4a
  > :S "(1 2 3) (a b c)"
  > inic
...
* TEST b4s.eval.nums : =eval= 1: numbers
#+begin_src b4a
  > :S "1" ..
...
* TEST b4s.eval.syms : =eval= 2: symbols get looked up in the environment
#+begin_src b4a
  > :S "((a . 1) (b . B)) a b c" ..
  > inic pre rval zp !E
...
* TEST b4s.eval.quo : =eval= 3: quote
#+begin_src b4a
...
* TEST b4s.eval.eq-p : =eval= 4: =eq?= and =t=
#+begin_src b4a
  > :S "((a . 1 ) (b . 1) (c . 2))"
  > inic pre rval zp !E
  > : "(eq? 1 2)"
...
* TEST b4s.eval.car : =eval= 5: (car (x y)) -> x
#+begin_src b4a
  > :S "(car '(a . b))"
  > inic pre rval zp nil eval pval \0
  a
...
* TEST b4s.eval.cdr : =eval= 6: (cdr (x y)) -> y
#+begin_src b4a
  > :S "(cdr '(a . b))"
  > inic pre rval zp nil eval pval \0
  b
...
* TEST b4s.eval.sym-p : =eval= 7: (sym? x)
#+begin_src b4a
  > :S "((a . 1) (b . b))"
  > inic pre rval zp !E
  > : "(sym? 'a)"
...
* TEST b4s.eval.num-p : =eval= 8: (num? x)
#+begin_src b4a
  > :S "((a . 1) (b . b))"
  > inic pre rval zp !E
  > : "(num? 'a)"
...
* TEST b4s.eval.cons : =eval= 9: (cons x y)
#+begin_src b4a
  > :S "((a . 1) (b . b))"
  > inic pre rval zp !E
  > : "(cons a b)"
...
* TEST b4s.elet : =elet= : eval helper for (let)
#+begin_src b4a
  > :S "((a . 1) (b . b))"
  > inic pre rval  zp !E
  > : "((a . 2) (b . a))"
...
* TEST b4s.eval.let : =eval= A: (let ((k . v)) x)
#+begin_src b4a
  > :S "((a . 1) (b . b))"
  > inic pre rval  zp !E
  > : "(let ((a . 2) (b . a)) a)"
...
* TEST b4s.econ : =econ= : eval helper for (cond)
#+begin_src b4a
  > :S "((p . t) (q))"
  > inic pre rval zp !E
  > : "((q 123) (p 'xyz))"
...
* TEST b4s.eval.cond : =eval= B: (cond)
#+begin_src b4a
...
* TEST b4s.zip : =zip= : eval helper for lambdas
#+begin_src b4a
  > :S "(a b c d e) (1 2 3 4)"
...
* TEST b4s.eval.lam : =eval= C: function applications
#+begin_src b4a
  > :S "((lam () C0))"
  > inic pre rval zp nil eval pval \0
  C0
