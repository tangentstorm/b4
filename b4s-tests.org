...
* TEST b4s.nxt^ : =inic= and =nxt^= : pointer to next free cell
#+begin_src b4a
  > inic
...
* TEST b4s.eq? : =eq?=  (=eq= is a b4 opcode)
#+begin_src b4a
> inic
...
* TEST b4s.eval.eq? : =eval= 4: =eq?= and =t=
#+begin_src b4a
  > :S "((a . 1 ) (b . 1) (c . 2))"
  > inic pre rval zp !E
  > : "(eq? 1 2)"
...
* TEST b4s.eval.sym? : =eval= 7: (sym? x)
#+begin_src b4a
  > :S "((a . 1) (b . b))"
  > inic pre rval zp !E
  > : "(sym? 'a)"
...
* TEST b4s.eval.num? : =eval= 8: (num? x)
#+begin_src b4a
  > :S "((a . 1) (b . b))"
  > inic pre rval zp !E
  > : "(num? 'a)"
