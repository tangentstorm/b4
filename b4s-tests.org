...
#+begin_src b4a
  ...
  ()
  > # if m1 @R := cnum(@N)
  > C1 m1 rsym @R pval \0 cscr
  bcde
  > :S 'x ') ..
  > C2 inic pre msym rsym @R pval \0
  x
#+end_src
...
* =node=, =newc=, =acl!=, and =done= : imperative list builder
#+name: b4s.rlis
#+begin_src b4a
  > inic
  > node @R pval \0 cscr
  (())
  > c1 cnum acl! @R pval \0 cscr
  (1 ())
  > c2 cnum acl! @R pval \0 cscr
  (1 2 ())
  > done
  > @R pval \0 cscr go00
  (1 2)
...
- @R holds the list we're building
- @L holds the last cell in the list
- @P is the previous cell
-acl! appends to the end of the list
  - the list always has an extra (()) at the end (that's @L)
  - when we append, we set its car, set P=L, and set L to a new cel in P's cdr
-end! ends the list by replacing P's link to L with its cdr
- final result stays in @R
- must preserve @R, @P, and @L when entering nested lists
